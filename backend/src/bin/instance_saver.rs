use backend::testing_common::instances::{
    challenge_instance, challenge_instance_multiple_dispatch, minimal_challenge_instance,
    transaction_instance, transactions_expected_from_challenge_instance,
};
...
fn main() -> std::io::Result<()> {
    ...
    save_instance(
        &challenge_instance_multiple_dispatch(),
        "challenge_instance_multiple_dispatch",
    )?;
    save_instance(&transaction_instance(), "transaction_instance")?;
    save_instance(
        &transactions_expected_from_challenge_instance(),
        "transactions_expected_from_challenge_instance",
    )?;
